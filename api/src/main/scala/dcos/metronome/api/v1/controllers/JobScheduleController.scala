...
class JobScheduleController(
  ...
  def updateSchedule(id: JobId, scheduleId: String) = AuthorizedAction.async(validate.jsonWithValidator(ScheduleSpec.validScheduleSpecWithId(scheduleId))) { implicit request =>

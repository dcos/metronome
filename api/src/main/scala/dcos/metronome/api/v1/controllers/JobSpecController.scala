...
import dcos.metronome.api.{ ApiConfig, AuthorizedController, ErrorDetail, UnknownJob }
...
class JobSpecController(
  ...
  def updateJob(id: JobId) = measuredAction {
  ...
  def deleteJob(id: JobId, stopCurrentJobRuns: Boolean) = measuredAction {
    AuthorizedAction.async { implicit request =>

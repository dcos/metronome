...
class ZkJobRunRepositoryTest extends FunSuite with Mockito with Matchers {
  ...
  test("ids") {
    ...
    f.store.allIds("job-runs").returns(Future.successful(Seq("first.job", "second.job")))

    f.store.allIds("job-runs/first.job").returns(Future.successful(Seq("first.run", "second.run")))

    f.store.allIds("job-runs/second.job").returns(Future.successful(Seq("third.run", "fourth.run")))

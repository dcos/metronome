...
class NetworkValidationTest extends UnitTest with ValidationTestLike {
  ...
  "host networking validation" should {
    "require that labels and name are not set" in {
      validate(newNetwork(mode = Network.NetworkMode.Host, name = Some("name"), labels = Map("key" -> "value"))) should haveViolations(
  ...
  "bridge networking validation" should {
    ...
    "allow specifications of label" in {
      validate(newNetwork(mode = Network.NetworkMode.ContainerBridge, labels = Map("key" -> "value"))) should be(aSuccess)
    }
  }
  ...
  "container networking validation" should {
    ...
    "should allow the specification of labels" in {
      validate(newNetwork(mode = Network.NetworkMode.Container, name = Some("labels"), labels = Map("key" -> "value"))) should be(aSuccess)
    }
  }

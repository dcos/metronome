...
object SecretDef {
  ...
  implicit object playJsonFormat extends play.api.libs.json.Format[SecretDef] {
    def reads(json: play.api.libs.json.JsValue): play.api.libs.json.JsResult[SecretDef] = {
      val source = json
        .\("source")
        .validate[String](play.api.libs.json.JsPath.read[String](minLength[String](ConstraintSourceMinLength)))
      val _errors = Seq(("source", source)).collect({
        case (field, e: play.api.libs.json.JsError) =>
          e.repath(play.api.libs.json.JsPath.\(field)).asInstanceOf[play.api.libs.json.JsError]
      })

...
object JobRunId {
  ...
  // TODO: parsing runSpecId to extract a JobRunId is not really clean. It would be better to let the
  ...
  def apply(runSpecId: PathId): JobRunId = {
    runSpecId.toString() match {
      case RunSpecRegex(jobSpecId, jobRunValue) => JobRunId(PathId(jobSpecId), jobRunValue)
      case _                                    => throw new MatchError(s"runSpecId $runSpecId is no valid identifier")
    }
  }
}

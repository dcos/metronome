...
object JobRunSpec {
  ...
  private def networkingValidation(isUcr: Boolean, isDocker: Boolean) = genericValidator[Seq[Network]] { networks =>
  ...
  implicit val validJobRunSpec = validator[JobRunSpec] { jobRunSpec =>
    ...
    jobRunSpec.gpus is isTrue[Int](JobRunSpecMessages.gpusNotValidWithDocker) { gpus => gpus == 0 || jobRunSpec.docker.isEmpty }
...
object JobRunSpecMessages {
  ...
  val mixedNetworkModesNotAllowed = "You may not mix host, container, or container/bridge networking. Only define one kind of network."

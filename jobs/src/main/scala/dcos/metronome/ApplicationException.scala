...
case class JobSpecWithNoSchedule(id: JobId)
    extends ApplicationException(s"We were expecting the job to have a schedule but it does not have any: $id")
...
case class ConcurrentJobRunNotAllowed(spec: JobSpec)
    extends ApplicationException(s"Concurrent JobRun not allowed ${spec.id}")

case class PersistenceFailed(id: String, reason: String)
    extends ApplicationException(s"Persistence Failed for: $id Reason: $reason")

case class UnexpectedTaskState(task: Task)
    extends ApplicationException(s"Encountered unexpected task state in repository: $task")

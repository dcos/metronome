...
def bundleLogGzipped(masterPath: Path) =
  masterPath / "dcos-metronome.service.gz"

def bundleLogGunzipped(masterPath: Path) =
  masterPath / "dcos-metronome.service"
...
def generateLogstashConfig(inputPath: Path, targetPath: Path, logFormat: LogFormat, files: Map[String, Path]) = {
  ...
  writeFiles(
    ...
    printing / "20-filters.conf" -> (read!(pwd / "conf" / "filter-metronome.conf")),
  ...
  writeFiles(
    ...
    loading / "20-filters.conf" -> (read!(pwd / "conf" / "filter-metronome.conf")),
  ...
  writeFiles(
    ...
    json / "20-filters.conf" -> (read!(pwd / "conf" / "filter-metronome.conf")),
